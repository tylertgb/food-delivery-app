  ...
  return (
    <div key={id} className='flex flex-col items-start shadow rounded-2xl'>
        <div className='overflow-hidden rounded-2xl relative'>
            <div className='z-10 absolute bottom-0 w-full h-20 bg-gradient-to-t from-orange-400 to-transparent'><span className='h-10 w-20 rounded-tr-2xl rounded-br-2xl flex items-center justify-center bg-white text-black text-xl font-semibold'>${price.toFixed(2)}</span></div>
            ...
            <div className='absolute top-0 right-3 bg-white rounded-full flex items-center p-1 mt-3'>
                {!cartItems[id] ? <img src={assets.add_icon_white} alt="" onClick={() => addToCart(id)} className='w-10' /> 
                : 
        ...
        <div className='flex flex-col gap-2 p-4'>
            ...
            <p className='text-xl font-semibold text-orange-500'>${price.toFixed(2)}</p>
        </div>
    </div>
  )
}
